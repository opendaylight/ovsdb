...
* Upgrade scenario (replay based upgrade)
   ...
   - existing switch connects back: we will not do reconciliation till the upgrade complete
     flag is set, once this flag is set, reconciliation kicks in and switch is published to OperDS

   - existing switch connects back(auto-tunnel): wait for upgrade to complete, reconcile (we may
     not have any configuration in config DS except TZ, that means we will delete every tep
     created by the controller in the switch) then publish to Oper DS. This may result in
     traffic loss.
