...
public class HwvtepDataChangeListener implements DataTreeChangeListener<Node>, AutoCloseable {
    ...
    private Map<HwvtepConnectionInstance, Collection<DataTreeModification<Node>>> changesByConnectionInstance(
		...
		for (DataTreeModification<Node> change : changes) {
		    ...
		    HwvtepConnectionInstance connection = getConnectionInstance(node);
		    if (connection != null) {
    ...
    private HwvtepConnectionInstance getConnectionInstance(Node node) {
        ...
        HwvtepConnectionInstance client = null;
        HwvtepGlobalAugmentation global = node.getAugmentation(HwvtepGlobalAugmentation.class);
        if (global != null) {
            client = hcm.getConnectionInstance(global.getConnectionInfo());
        } else {
            HwvtepLogicalSwitchAugmentation logicalSwitch = node.getAugmentation(HwvtepLogicalSwitchAugmentation.class);
            if (logicalSwitch != null ) {
                //The format of node-id is fixed. It will be one of the two:
                //    1 Switch initiates connection: In this case node-id will be 'hwvtep://uuid/<uuid of switch>'
                //    2 User initiates connection from ODL: In this case node-id will be 'hwvtep://ip:port'
                String nodeId = node.getNodeId().toString();
                if (nodeId.indexOf("uuid") != -1) {
                    //Case 2 User initiates connection
                    Pattern pattern = Pattern.compile("[0-9a-z]{8}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{4}-[0-9a-z]{12}");
                    Matcher matcher = pattern.matcher(nodeId);
                    if (matcher.find()) {
                        //TODO
                        //Get the connection instance
                    } else {
                        LOG.warn("Failed to match the node-id {}", nodeId);
                    }
                } else {
                    Pattern pattern = Pattern.compile("((1?\\d{1,2}|2[1-5][1-5])\\.){3}(1?\\d{1,2}|2[1-5][1-5]):([1-5]?\\d{1,4}|6[1-5][1-5][1-3][1-5])");
                    Matcher matcher = pattern.matcher(nodeId);
                    if (matcher.find()) {
                        int pos = nodeId.indexOf(":", matcher.start());
                        String ip = nodeId.substring(matcher.start()-1, pos);
                        String port = nodeId.substring(pos+1, matcher.end());
                        IpAddress remoteIP = new IpAddress(ip.toCharArray());
                        PortNumber remotePort = new PortNumber(Integer.valueOf(port));
                        ConnectionInfo key = new ConnectionInfoBuilder().setRemoteIp(remoteIP).setRemotePort(remotePort).build();
                        client = hcm.getConnectionInstance(key);
                    } else {
                        LOG.warn("Failed to match the node-id {}", nodeId);
                    }
                }
