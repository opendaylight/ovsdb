...
public class D1ConnectedHandler implements HAStateHandler {
    ...
    public void handle(HAContext ctx, ReadWriteTransaction tx) {
        try {
            LOG.error("D1 connected haId {} nodeId {}",
                    ctx.getHaId(), ctx.nodeIdOptional.get().getValue());
            LOG.error("D1 connected build global config node managers for haId {}",
                    ctx.getHaId());
            LOG.error("D1 connected copy operational from node {} haId {}",
                    ctx.nodeIdOptional.get().getValue(),
                    ctx.getHaId());
            LOG.error("D1 connected copy switch operational from node {} haId {}",
                    ctx.nodeIdOptional.get().getValue(),
                    ctx.getHaId());
