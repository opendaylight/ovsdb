...
public class D2ConnectedHandler implements HAStateHandler {
    ...
    public void handle(final HAContext ctx, final ReadWriteTransaction tx) {
        try {
            LOG.error("D2 connected haId {} nodeId {}",
                    ctx.getHaId(), ctx.nodeIdOptional.get().getValue());

            LOG.error("D2 connected build global config node managers for haId {}",
                    ctx.getHaId());
            LOG.error("D2 connected copy operational from node {} haId {}",
                    ctx.nodeIdOptional.get().getValue(),
                    ctx.getHaId());
            LOG.error("D2 connected copy switch operational from node {} haId {}",
                    ctx.nodeIdOptional.get().getValue(),
                    ctx.getHaId());

            LOG.error("D2 connected copy config from haId {} to {}",
                    ctx.getHaId(),
                    ctx.nodeIdOptional.get().getValue());
    ...
    public static void onChildGlobalOperationalUpdate(Node updated, Node original,
        ...
        logicalSwitchWaitingNodes.remove(updated.getNodeId().getValue());
