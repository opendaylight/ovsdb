...
public class ReconcileOpNodeListener implements ClusteredDataTreeChangeListener<Node>, AutoCloseable {

    private ListenerRegistration<ReconcileOpNodeListener> registration;
    private HwvtepConnectionManager hcm;
    private DataBroker db;
    private static final Logger LOG = LoggerFactory.getLogger(ReconcileOpNodeListener.class);

    public ReconcileOpNodeListener(DataBroker db, HwvtepConnectionManager hcm) {
    ...
    private void registerListener(final DataBroker db) {
        ...
        try {
            ...
            registration = db.registerDataTreeChangeListener(treeId, ReconcileOpNodeListener.this);
        ...
        for (DataTreeModification<Node> change : changes) {
            ...
            if (deleted != null) {
                if (deleted.getAugmentation(HwvtepGlobalAugmentation.class) != null) {
                    LOG.error("cancel reconcile config tid {} for node {}", HAUtil.getId(),
                            deleted.getNodeId().getValue());
        ...
        for (DataTreeModification<Node> change : changes) {
            ...
            if (node != null) {
                ...
                if (physicalSwitch != null) {
                    ...
                    if (connection != null) {
                        LOG.error("reconcile config tid {} for node {} from {}",
                                HAUtil.getId(), node.getKey(),
