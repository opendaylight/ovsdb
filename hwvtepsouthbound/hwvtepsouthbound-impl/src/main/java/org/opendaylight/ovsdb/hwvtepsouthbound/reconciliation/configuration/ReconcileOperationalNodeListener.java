...
public class ReconcileOperationalNodeListener implements ClusteredDataTreeChangeListener<Node>, AutoCloseable {

    private ListenerRegistration<ReconcileOperationalNodeListener> registration;
    private HwvtepConnectionManager hcm;
    private DataBroker db;
    private static final Logger LOG = LoggerFactory.getLogger(ReconcileOperationalNodeListener.class);

    public ReconcileOperationalNodeListener(DataBroker db, HwvtepConnectionManager hcm) {
        LOG.trace("Registering ReconcileOperationalNodeListener for operational nodes");
        this.db = db;
        this.hcm = hcm;
        registerListener(db);
    }
    ...
    private void registerListener(final DataBroker db) {
        ...
        try {
            ...
            registration = db.registerDataTreeChangeListener(treeId, ReconcileOperationalNodeListener.this);
        } catch (final Exception e) {
            LOG.warn("ReconcileOperationalNodeListener registration failed", e);
        }
    }
        ...
        for (DataTreeModification<Node> change : changes) {
            ...
            if (deleted != null) {
                if (deleted.getAugmentation(HwvtepGlobalAugmentation.class) != null) {
                    LOG.trace("cancel reconcile config for node {}", deleted.getKey());
