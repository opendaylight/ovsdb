...
public class HwvtepOperationalState {
    ...
    public HwvtepOperationalState(DataBroker db, Collection<DataTreeModification<Node>> changes) {
        ...
        if (nodeCreateOrUpdate != null) {
            for (Entry<InstanceIdentifier<Node>, Node> entry: nodeCreateOrUpdate.entrySet()) {
                Node readNode = readNodeFromDatastore(db, entry.getKey());
                ...
                if (readNode != null) {
                    HwvtepGlobalAugmentation hgAugmentation = readNode.getAugmentation(HwvtepGlobalAugmentation.class);
                    PhysicalSwitchAugmentation psAugmentation = readNode.getAugmentation(PhysicalSwitchAugmentation.class);
    ...
    private Node readNodeFromDatastore(DataBroker db, InstanceIdentifier<Node> iid) {
        ...
        try {
            ...
            if (nodeOptional.isPresent()) {
                operationalNodes.put(iid, nodeOptional.get());
                result = nodeOptional.get();
            }
