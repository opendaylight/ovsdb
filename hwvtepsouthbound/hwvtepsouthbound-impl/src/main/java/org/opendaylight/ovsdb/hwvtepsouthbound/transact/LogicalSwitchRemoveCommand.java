    ...
    public void execute(TransactionBuilder transaction) {
        ...
        if (!removeds.isEmpty()) {
            for (Entry<InstanceIdentifier<Node>, List<LogicalSwitches>> created:
                removeds.entrySet()) {
                deleteLogicalSwitch(transaction,  created.getKey(), created.getValue());
            }
        }
    }

    private void deleteLogicalSwitch(TransactionBuilder transaction,
            InstanceIdentifier<Node> instanceIdentifier, List<LogicalSwitches> lswitchList) {
        for (LogicalSwitches lswitch: lswitchList) {
            LOG.debug("Deleting logcial switch named: {}", lswitch.getHwvtepNodeName().getValue());
            ...
            if (operationalSwitchOptional.isPresent() &&
                ...
                transaction.add(op.delete(logicalSwitch.getSchema())
                        .where(logicalSwitch.getNameColumn().getSchema().opEqual(lswitch.getHwvtepNodeName().getValue())).build());
                transaction.add(op.comment("Logical Switch: Deleting " + operationalSwitchOptional.get().getHwvtepNodeName().getValue()));
            } else {
                LOG.warn("Unable to delete logical switch {} because it was not found in the operational store, "
                        + "and thus we cannot retrieve its UUID", lswitch.getHwvtepNodeName().getValue());
            }
        }
    }
