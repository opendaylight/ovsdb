...
public class GlobalUpdateCommand extends AbstractTransactionCommand {
    ...
    public void execute(ReadWriteTransaction transaction) {
        Map<UUID, Global> updatedHwvtepRows = TyperUtils
                        .extractRowsUpdated(Global.class, getUpdates(),
                                getDbSchema());
                Map<UUID, Global> deletedHwvtepRows = TyperUtils
                        .extractRowsOld(Global.class, getUpdates(),
                                getDbSchema());

                for (Entry<UUID, Global> entry : updatedHwvtepRows.entrySet()) {
                    Global hwvtepGlobal = entry.getValue();
                    Global oldEntry = deletedHwvtepRows.get(entry.getKey());
    ...
    private InstanceIdentifier<Node> getInstanceIdentifier(Global hwvtep) {
        InstanceIdentifier<Node> iid = HwvtepSouthboundMapper.getInstanceIdentifier(hwvtep);
