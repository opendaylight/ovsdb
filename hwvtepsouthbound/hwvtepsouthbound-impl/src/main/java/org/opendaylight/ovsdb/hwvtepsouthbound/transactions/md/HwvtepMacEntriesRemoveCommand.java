    ...
    private void removeUcastMacsLocal(ReadWriteTransaction transaction) {
        ...
        for (UcastMacsLocal lum : deletedLUMRows) {
            if (lum.getMac() != null && lum.getLogicalSwitchColumn() != null
                ...
                InstanceIdentifier<LocalUcastMacs> lumId = getDeviceOpDataKey(LocalUcastMacs.class, lum.getUuid());
                if (lumId == null) {
                    if (getLogicalSwitchRef(lum.getLogicalSwitchColumn().getData()) == null) {
                        continue;
                    }
                    lumId = getOvsdbConnectionInstance().getInstanceIdentifier()
                ...
                getOvsdbConnectionInstance().getDeviceInfo().clearDeviceOperData(LocalUcastMacs.class, lumId);
            } else {
    ...
    private void removeUcastMacsRemote(ReadWriteTransaction transaction) {
        ...
        for (UcastMacsRemote rum : deletedUMRRows) {
            if (rum.getMac() != null && rum.getLogicalSwitchColumn() != null
                ...
                InstanceIdentifier<RemoteUcastMacs> rumId = getDeviceOpDataKey(RemoteUcastMacs.class, rum.getUuid());
                if (rumId == null) {
                    if (getLogicalSwitchRef(rum.getLogicalSwitchColumn().getData()) == null) {
                        continue;
                    }
                    rumId = getOvsdbConnectionInstance().getInstanceIdentifier()
    ...
    private void removeMcastMacsLocal(ReadWriteTransaction transaction) {
        ...
        for (McastMacsLocal lmm : deletedLMMRows) {
            if (lmm.getMac() != null && lmm.getLogicalSwitchColumn() != null
                ...
                InstanceIdentifier<LocalMcastMacs> lumId = getDeviceOpDataKey(LocalMcastMacs.class, lmm.getUuid());
                if (lumId == null) {
                    if (getLogicalSwitchRef(lmm.getLogicalSwitchColumn().getData()) == null) {
                        continue;
                    }
                    lumId = getOvsdbConnectionInstance().getInstanceIdentifier()
                ...
                transaction.delete(LogicalDatastoreType.OPERATIONAL, lumId);
                getOvsdbConnectionInstance().getDeviceInfo().clearDeviceOperData(LocalMcastMacs.class, lumId);
            } else {
    ...
    private void removeMcastMacsRemote(ReadWriteTransaction transaction) {
        ...
        for (McastMacsRemote rmm : deletedMMRRows) {
            if (rmm.getMac() != null && rmm.getLogicalSwitchColumn() != null
                ...
                InstanceIdentifier<RemoteMcastMacs> lumId = getDeviceOpDataKey(RemoteMcastMacs.class, rmm.getUuid());
                if (lumId == null) {
                    if (getLogicalSwitchRef(rmm.getLogicalSwitchColumn().getData()) == null) {
                        continue;
                    }
                    lumId = getOvsdbConnectionInstance().getInstanceIdentifier()
                            .augmentation(HwvtepGlobalAugmentation.class)
