...
public class HwvtepPhysicalPortUpdateCommand extends AbstractTransactionCommand {
    ...
    private void updateTerminationPoints(ReadWriteTransaction transaction, Node node) {
        for (Entry<UUID, PhysicalPort> pPortUpdateEntry : updatedPPRows.entrySet()) {
            ...
            if (switchIid.isPresent()) {
                ...
                deleteEntries(transaction,PortFaultStatusToRemove( tpPath, pPortUpdate));
            }
        }
    }
    ...
    private void setPortFaultStatus(HwvtepPhysicalPortAugmentationBuilder tpAugmentationBuilder, PhysicalPort portUpdate) {
        if (portUpdate.getPortFaultStatusColumn() != null && portUpdate.getPortFaultStatusColumn().getData() != null
            ...
            for (String portFaultStatus : portUpdate.getPortFaultStatusColumn().getData()) {
                portFaultStatusLst.add(
                        new PortFaultStatusBuilder().setKey(new PortFaultStatusKey(portFaultStatus)).setPortFaultStatusKey(portFaultStatus).build());
            }
    ...
    private List<InstanceIdentifier<PortFaultStatus>> PortFaultStatusToRemove(InstanceIdentifier<TerminationPoint> tpPath,PhysicalPort pPort){
        ...
        Preconditions.checkNotNull(pPort);
    	 ...
    	 if (oldPort != null && oldPort.getPortFaultStatusColumn() != null) {
    		  for (String portFltStat:
    			  oldPort.getPortFaultStatusColumn().getData()) {
                  if (pPort.getPortFaultStatusColumn() == null
                          || !pPort.getPortFaultStatusColumn().getData().contains(portFltStat)) {
                      InstanceIdentifier<PortFaultStatus> iid = tpPath
                              .augmentation(HwvtepPhysicalPortAugmentation.class)
                              .child(PortFaultStatus.class,
                            		  new PortFaultStatusKey(portFltStat));
                      result.add(iid);
                  }
              }
    	 }
