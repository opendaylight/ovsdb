...
public class HwvtepPhysicalSwitchUpdateCommand extends AbstractTransactionCommand {
    ...
    private void updatePhysicalSwitch(ReadWriteTransaction transaction, PhysicalSwitch pSwitch) {
        ...
        if (connection.isPresent()) {
            ...
            deleteEntries(transaction, SwitchFaultStatusToRemove(psIid,pSwitch));
        }
    }
    ...
    private List<InstanceIdentifier<SwitchFaultStatus>> SwitchFaultStatusToRemove(
            InstanceIdentifier<Node> psIid, PhysicalSwitch pSwitch) {
        ...
        if (oldSwitch != null && oldSwitch.getSwitchFaultStatusColumn() != null) {
            for (String switchFltStat:
            	oldSwitch.getSwitchFaultStatusColumn().getData()) {
                if (pSwitch.getSwitchFaultStatusColumn() == null
                        || !pSwitch.getSwitchFaultStatusColumn().getData().contains(switchFltStat)) {
                    InstanceIdentifier<SwitchFaultStatus> iid = psIid
                            .augmentation(PhysicalSwitchAugmentation.class)
                            .child(SwitchFaultStatus.class,
                                    new SwitchFaultStatusKey(switchFltStat));
                    result.add(iid);
                }
            }
        }
    ...
    private void setSwitchFaultStatus(PhysicalSwitchAugmentationBuilder psAugmentationBuilder, PhysicalSwitch pSwitch) {
        if (pSwitch.getSwitchFaultStatusColumn() != null && pSwitch.getSwitchFaultStatusColumn().getData() != null
            ...
            for (String switchFaultStatus : pSwitch.getSwitchFaultStatusColumn().getData()) {
             ...
             switchFaultStatusLst.add(
                        new SwitchFaultStatusBuilder().setKey(new SwitchFaultStatusKey(switchFaultStatus)).setSwitchFaultStatusKey(switchFaultStatus).build());
            }
