...
final class RealBaseType extends BaseType<RealBaseType> {
    ...
    private static final class Factory extends BaseTypeFactory<RealBaseType> {
        @Override
        ...
        private static ImmutableSet<Double> parseEnums(final JsonNode node) {
            final List<Double> tmp = new ArrayList<>();
            for (JsonNode enm : node.get(1)) {
                tmp.add(enm.asDouble());
            }
