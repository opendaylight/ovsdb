...
public final class TypedReflections {
    ...
    static Range<Version> createVersionRange(final String from, final String until) {
        return createVersionRange(from == null ? Version.NULL : Version.fromString(from),
                until == null ? Version.NULL : Version.fromString(until));
    }

    @VisibleForTesting
    static Range<Version> createVersionRange(final Version from, final Version to) {
        if (Version.NULL.equals(from)) {
            return Version.NULL.equals(to) ? Range.all() : Range.atMost(to);
        }
        return Version.NULL.equals(to) ? Range.atLeast(from) : Range.closed(from, to);
    }
}
