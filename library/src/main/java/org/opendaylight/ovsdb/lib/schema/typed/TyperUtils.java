        ...
        return Reflection.newProxy(klazz, new InvocationHandler() {
            ...
            private Object processGetColumn(Method method) throws Throwable {
                ...
                ColumnSchema<GenericTableSchema, Object> columnSchema =
                        getColumnSchema(tableSchema, columnName, (Class<Object>) method.getReturnType());
                if (columnSchema == null) {
                    //try to mitigate by returning a generic column that has an empty
                    //Set<String> as data if the UNKNOWN_COLUMN is null, it means this
                    //is the first time we hit this condition hence instantiate using
                    //the getBridgesColumn method.
                    if (UNKNOWN_COLUMN == null) {
                        try {
                            ...
                            //If we get here it means that the very basic method of ovsdb, the
                            //get bridges method, does not exist. In that case, this ovsdb is
                            //almost completely out of sync with the schema we expect, hence
                            //continue to the original implementation of throwing an exception
                            //about the unknown column.
                        }
                    }
                    //If the UNKNOWN_COLUMN is not null, we may be able to mitigate the
                    //missing column from the current schema.
                    if (UNKNOWN_COLUMN != null) {
