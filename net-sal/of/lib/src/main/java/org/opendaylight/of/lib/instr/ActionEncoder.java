...
class ActionEncoder {
    ...
    // If we have an appropriate factory for the experimenter ID, use that;
    // else if the action is the generic class, encode the data bytes as is;
    // else throw an unhandled action exception.
    private static void encodeExperAction(ActExperimenter act,
                                          OfPacketWriter pkt) {
        pkt.writeInt(act.id);
        ExtActionFactory extFactory = ActionFactory.getExtFactory(act.id);

        if (extFactory != null) {
            extFactory.encodeActionInstance(act, pkt);
            return;
        }

        if (act.getClass() != ActExperimenter.class)
            throw new IllegalStateException(E_UNHANDLED_ACTION +
                    act.id + "," + act.getClass().getName());
        // assumption is that data is a valid length (multiple of 8)
        pkt.writeBytes(act.data);
    }

    private static final String E_UNHANDLED_ACTION =
            "Unhandled action:experimenter-id=";
}