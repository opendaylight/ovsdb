...
public class ActionFactory extends AbstractFactory {
    ...
    private static final String E_ALREADY_REGISTERED =
            "Extension Action Factory already registered for id: ";

    /*
     * DESIGN NOTE: using Integer (and not ExperimenterId) as the key, so
     * ID's other than those officially sanctioned by ONF (that we have
     * compiled into ExperimenterId) can be supported.
     */
    private static final Map<Integer, ExtActionFactory> extensions =
            new HashMap<>();


    /**
     * Registers an extension to the action factory. The specified factory is
     * declaring that it should handle experimenter actions for the given
     * experimenter ID.
     *
     * @param expId the experimenter ID
     * @param factory the factory instance
     * @throws IllegalStateException if a binding already exists
     */
    public static void registerExtension(ExperimenterId expId,
                                         ExtActionFactory factory) {
        registerExtension(expId.encodedId(), factory);
    }
     ...
     * @throws IllegalStateException if a binding already exists
     */
    public static void registerExtension(int expId, ExtActionFactory factory) {
        if (extensions.get(expId) != null)
            throw new IllegalStateException(E_ALREADY_REGISTERED + hex(expId));
        extensions.put(expId, factory);
    }

    /**
     * Returns the extension action factory for the specified experimenter ID,
     * if one has been registered.
     *
     * @param expId the experimenter ID
     * @return the corresponding extension action factory
     */
    public static ExtActionFactory getExtFactory(int expId) {
        return extensions.get(expId);
    }

    /**
     * Removes the binding for an extension action factory from the given
     * experimenter ID.
     *
     * @param expId the experimenter ID
     */
    public static void unregisterExtension(ExperimenterId expId) {
        unregisterExtension(expId.encodedId());
    }
     ...
     * @param expId the experimenter ID
     */
    public static void unregisterExtension(int expId) {
        extensions.remove(expId);
    }
