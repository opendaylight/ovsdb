...
class ActionParser {
    ...
    private static Action parseActionHeader(OfPacketReader pkt,
        ...
        try {
            ...
            switch (header.type) {
                case EXPERIMENTER:
                    // TODO: warning - may have to integrate extension factories
...
//                                E_UNEXPECTED_HEADER_LENGTH + header.length);
    ...
    private static Action createActionInstance(Action.Header header,
        ...
        switch (header.type) {
            ...
            case EXPERIMENTER:
                try {
                    action = generateExperAction(pv, header, pkt);
                } catch (DecodeException e) {
                    throw AF.mpe(pkt, E_EXP_DECODE, e);
                }
                break;
        }
        return action;
    ...
    private static final String E_EXP_DECODE =
            "Experimeter Action decode failure";
    ...
    // Generate an experimenter action. If we have an appropriate factory for
    // the experimenter ID, use that; else return a generic action structure.
    private static Action generateExperAction(ProtocolVersion pv,
                                              Action.Header header,
                                              OfPacketReader pkt)
            throws DecodeException {
        int expIdCode = pkt.readInt();
        ExtActionFactory extFactory = ActionFactory.getExtFactory(expIdCode);

        if (extFactory != null)
            return extFactory.createActionInstance(pv, header, expIdCode, pkt);

        return experAction(new ActExperimenter(pv, header), expIdCode, pkt);
    }

    // Completes parsing a default EXPERIMENTER action.
    private static Action experAction(ActExperimenter act, OfPacketReader pkt) {
        return experAction(act, pkt.readInt(), pkt);
    }

    // Completes parsing a default EXPERIMENTER action.
    private static Action experAction(ActExperimenter act, int expIdCode,
                                      OfPacketReader pkt) {
        act.id = expIdCode;
