...
public class InternalNetworkManager {
    ...
    public boolean isPortOnBridge (Node node, Bridge bridge, String ifName) {
        OVSDBConfigService ovsdbTable = (OVSDBConfigService) ServiceHelper.getGlobalInstance(OVSDBConfigService.class, this);

        for (UUID portsUUID : bridge.getPorts()) {
            try {
                Port port = (Port) ovsdbTable.getRow(node, Port.NAME.getName(), portsUUID.toString());
                if ((port != null) && port.getName().equalsIgnoreCase(ifName)) {
                    return true;
                }
            } catch (Exception e) {
    ...
    public boolean isInternalNetworkVlanReady (Node node, NeutronNetwork network) {
        logger.trace("shague isInternalNetworkVlanReady: node: {}", node);
        /* is br-int created */
        Bridge intBridge = this.getInternalBridge(node, AdminConfigManager.getManager().getIntegrationBridgeName());
        if (intBridge == null) {
        ...
        if (ProviderNetworkManager.getManager().hasPerTenantTunneling()) {
            ...
            String phyNetName = AdminConfigManager.getManager().getPhyIfName(node, network.getProviderPhysicalNetwork());
       ...
       Bridge br-int
            Port br-int
                Interface br-int
                    type: internal
     */
    public void createIntegrationNetwork (Node node) throws Exception {
    ...
    public void prepareInternalNetwork(Node node) {
        logger.trace("shague prepareInternalNetwork node: {}", node);
        try {
            this.createIntegrationNetwork(node);
        } catch (Exception e) {
            logger.error("Error creating internal network "+node.toString(), e);
        }
        /* TODO: Madhu look at this since it looks redundant. notifyNode() is also called for every new node added. */
    ...
    public boolean checkAndCreateNetwork (Node node, NeutronNetwork network) {
        ...
        logger.trace("shague checkAndCreateNetwork node: {}, network: {}:{}",
                node, network.getNetworkUUID(),network.getNetworkName());
        if (network.getProviderNetworkType().equalsIgnoreCase("vlan")) {
