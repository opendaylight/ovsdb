...
public class NodeConfiguration {
    ...
    private void getNodeConfiguration(Node node) {
        ...
        int vlan = 0;
        NeutronNetwork networkId = new NeutronNetwork();
        OVSDBConfigService ovsdbTable = (OVSDBConfigService) ServiceHelper.getGlobalInstance(OVSDBConfigService.class, this);

        try {
            Map<String, Table<?>> portRows = ovsdbTable.getRows(node, Interface.NAME.getName());
            ...
            for (Table<?> row : portRows.values()) {
                ...
                for (UUID ifaceId : port.getInterfaces()) {
                    ...
                    networkId = TenantNetworkManager.getManager().getTenantNetworkForInterface(iface);
                ...
                if (vlan != 0 && networkId != null) {
                    ...
                    this.tenantVlanMap.put(networkId.toString(), vlan);

                }
            }
        }
        catch (Exception e) {
            logger.error("Error getting Port table for Node {}", node);
        }
    }
