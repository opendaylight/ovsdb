...
class OF10ProviderManager extends ProviderNetworkManager {
    ...
    private void programLocalEgressVlanBridgeRules (Node node, int inOFPort, int phyOFPort, int internalVlan, String segmentationId) {
        ...
        try {
            ...
            FlowConfig flowExtNet = new FlowConfig();
            String flowName2 = getVlanEgressPhyFlowName(phyOFPort, internalVlan, segmentationId);
            flowExtNet.setName(flowName2);
            flowExtNet.setNode(ofNode);
            flowExtNet.setInstallInHw(true);
            flowExtNet.setPriority(EGRESS_TUNNEL_FLOW_PRIORITY + "");
            flowExtNet.setIngressPort(phyOFPort + "");
            flowExtNet.setVlanId(segmentationId);
            List<String> actions2 = new ArrayList<String>();
            actions2.add(ActionType.OUTPUT.toString()+"="+inOFPort);
            flowExtNet.setActions(actions2);
            status = this.addStaticFlow(ofNode, flowExtNet);
            logger.debug("shague Local Egress Flow Programming Status {} for Flow {} on {} / {}",
                    status, flowExtNet, ofNode, node);
        } catch (Exception e) {
            logger.error("Failed to initialize Flow Rules for {}", node, e);
        }
    }
    ...
    private void removeLocalEgressVlanBridgeRules (Node node, int inOFPort, int phyOFPort, int internalVlan, String segmentationId) {
        ...
        logger.debug("shague: ingress: node: {}, inOfPort: {}, internalVlan: {}, segmentationId: {}, brNetId: {}",
                node, inOFPort, internalVlan, segmentationId, brNetId);
        try {
            OVSDBConfigService ovsdbTable = (OVSDBConfigService)ServiceHelper.getGlobalInstance(OVSDBConfigService.class, this);
            Bridge bridge = (Bridge) ovsdbTable.getRow(node, Bridge.NAME.getName(), brNetId);
            Set<String> dpids = bridge.getDatapath_id();
    ...
    public class vlanNet {
        ...
        public void initializeVlanNet (NeutronNetwork network, Node node, Interface intf) {
            logger.debug("shague getVlanNet: node: {}, intf: {}, networkUUID: {}", node, intf, network.getNetworkUUID());
    ...
    private void programVlanRules (NeutronNetwork network, Node node, Interface intf) {
        ...
        if (vlanNet.isValid()) {
            programLocalIngressVlanBridgeRules(node, vlanNet.patchIntOfPort, vlanNet.internalVlan, network.getProviderSegmentationID());
            programLocalEgressVlanBridgeRules(node, vlanNet.patchNetOfPort, vlanNet.phyOfPort, vlanNet.internalVlan, network.getProviderSegmentationID());
        }
    }
    ...
    private void removeVlanRules (NeutronNetwork network, Node node, Interface intf) {
        vlanNet vlanNet = new vlanNet(network, node, intf);
        if (vlanNet.isValid()) {
            removeLocalIngressVlanBridgeRules(node, vlanNet.patchIntOfPort, vlanNet.internalVlan, network.getProviderSegmentationID());
            removeLocalEgressVlanBridgeRules(node, vlanNet.patchNetOfPort, vlanNet.phyOfPort, vlanNet.internalVlan, network.getProviderSegmentationID());
        }
    }

    @Override
    public Status handleInterfaceUpdate(NeutronNetwork network, Node srcNode, Interface intf) {
        logger.debug("shague handleInterfaceUpdate: networkType: {}, segmentationId: {}, srcNode: {}, intf: {}",
    ...
    public Status handleInterfaceDelete(NeutronNetwork network, Node srcNode, Interface intf, boolean isLastInstanceOnNode) {
        ...
        logger.debug("shague handleInterfaceDelete: networkType: {}, segmentationId: {}, srcNode: {}, intf: {}",
