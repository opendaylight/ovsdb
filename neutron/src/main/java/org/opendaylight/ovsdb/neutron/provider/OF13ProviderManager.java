        ...
        try {
            ...
            if (intfs != null) {
                for (org.opendaylight.ovsdb.lib.table.internal.Table<?> row : intfs.values()) {
                    ...
                    if (ethIntf.getName().equalsIgnoreCase(AdminConfigManager.
                        getManager().getPhyIfName(node, network.getProviderPhysicalNetwork()))) {
    ...
    public Status handleInterfaceUpdate(NeutronNetwork network, Node srcNode, Interface intf) {
        ...
        if (network.getProviderNetworkType().equalsIgnoreCase("vlan")) {
            ...
            for (Node dstNode : nodes) {
                ...
                if (status.isSuccess()) {
                    this.programTunnelRules(network.getProviderNetworkType(),
                                            network.getProviderSegmentationID(),
                                            dst, srcNode, intf, true);
                }
                addTunnelPort(dstNode, network.getProviderNetworkType(), dst, src);
                if (status.isSuccess()) {
                    this.programTunnelRules(network.getProviderNetworkType(),
                                            network.getProviderSegmentationID(),
                                            src, dstNode, intf, false);
                }
            }
        }
