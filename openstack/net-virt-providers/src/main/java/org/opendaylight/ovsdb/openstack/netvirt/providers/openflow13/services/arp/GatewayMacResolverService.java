...
public class GatewayMacResolverService extends AbstractServiceInstance
    ...
    private final ConcurrentHashMap<Ipv4Address, ArpResolverMetadata> gatewayToArpMetadataMap =
    ...
    private final ScheduledExecutorService refreshRequester = Executors.newScheduledThreadPool(1);
            ...
            refreshRequester.scheduleWithFixedDelay(new Runnable(){
                ...
                public void run() {
                    if (!gatewayToArpMetadataMap.isEmpty()){
                        for(final Ipv4Address gatewayIp : gatewayToArpMetadataMap.keySet()){
                            final ArpResolverMetadata gatewayMetaData = gatewayToArpMetadataMap.get(gatewayIp);
