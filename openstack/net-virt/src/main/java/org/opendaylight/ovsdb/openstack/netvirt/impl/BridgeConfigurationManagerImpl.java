...
public class BridgeConfigurationManagerImpl implements BridgeConfigurationManager, ConfigInterface {
    ...
    private boolean createIntegrationToExternalPatch(Node bridgeNode) throws Exception {
        ...
        final boolean isIntegrationBridge = (southbound.getBridge(bridgeNode, brIntName) != null);
        if (! isIntegrationBridge && southbound.getBridge(bridgeNode, brExtName) == null) {
            // If we made it here, bridge node is neither integration nor external. Noop.
            return true;
        }
        ...
        if (isIntegrationBridge) {
            return addPatchPort(bridgeNode, brIntName, portNameInt, portNameExt);
        }
