...
public class BridgeConfigurationManagerImpl implements BridgeConfigurationManager, ConfigInterface {
     ...
     */
    private boolean createIntegrationBridge(Node node) throws Exception {
        Preconditions.checkNotNull(configurationService);

        if (!addBridge(node, configurationService.getIntegrationBridgeName())) {
    ...
    private boolean createExternalBridge(Node node) throws Exception {
        Preconditions.checkNotNull(configurationService);

        if (!addBridge(node, configurationService.getExternalBridgeName())) {
    ...
    private boolean createBridges(Node bridgeNode, Node ovsdbNode, NeutronNetwork network) throws Exception {
        ...
        if (isL3Enabled()) {
            ...
            if (!addPatchPort(bridgeNode, brInt, "extpatchport", "intpatchport")) {
