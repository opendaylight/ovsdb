...
public class NeutronL3Adapter {
    ...
    private Map<String, NeutronRouter_Interface> subnetId2RouterInterfaceCache;
    ...
    void init() {
        ...
        this.subnetId2RouterInterfaceCache = new HashMap<>();
    }
    ...
    public void handleNeutronPortEvent(final NeutronPort neutronPort, Action action) {
        ...
        // Treat the port event as a router interface event iff the port belongs to router. This is a
        ...
        if (neutronPort.getDeviceOwner().equalsIgnoreCase("network:router_interface")) {
            ...
            if (!isDelete) {
                for (Neutron_IPs neutronIP : neutronPort.getFixedIPs()) {
                    NeutronRouter_Interface neutronRouterInterface =
                            subnetId2RouterInterfaceCache.get(neutronIP.getSubnetUUID());
    ...
    private void programFlowsForNeutronRouterInterface(final NeutronRouter_Interface neutronRouterInterface,
        ...
        if (isDelete) {
            ...
            subnetId2RouterInterfaceCache.remove(subnet.getSubnetUUID());
        } else {
            networkId2RouterMacCache.put(neutronNetwork.getNetworkUUID(), macAddress);
            subnetId2RouterInterfaceCache.put(subnet.getSubnetUUID(), neutronRouterInterface);
        }
