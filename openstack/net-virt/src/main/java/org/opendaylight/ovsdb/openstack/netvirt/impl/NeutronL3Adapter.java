...
public class NeutronL3Adapter implements ConfigInterface {
    ...
    private void programFlowsForFloatingIP(final NeutronFloatingIP neutronFloatingIP, Boolean isDelete) {
        ...
        // FIXME!!! This needs to be revisited once floating ip is broken up
        if (isDelete || neutronFloatingIP.getFixedIPAddress() == null) {
            return;
        }
        ...
        for (Node node : nodes) {
            ...
            //
            if (externalSegmentation != null && !externalSegmentation.isEmpty()) {
    ...
    private void programIpRewriteStage1(Node node,
                                        Long dpid,
                                        String sourceSegmentationId,
                                        final boolean isInbound,
                                        String matchAddress,
                                        String destSegmentationId,
                                        String rewriteAddress,
                                        Action actionForNode) {
        // Based on the local cache, figure out whether programming needs to occur. To do this, we
        // will look at desired action for node.
        //
    ...
    private Status programIpRewriteStage2(Node node,
                                          Long dpid,
                                          String sourceSegmentationId,
                                          final boolean isInbound,
                                          String matchAddress,
