...
public class BridgeConfigurationManagerImplTest {
    ...
    public void testIsNodeTunnelReady() throws Exception {
        when(southbound.isBridgeOnOvsdbNode(node, ""))
                .thenReturn(false)
                .thenReturn(true);
        when(bridgeConfigurationManagerImpl.isNodeL3Ready(node, node))
                .thenReturn(true);

        verifyNoMoreInteractions(configurationService);
        assertFalse("Error, did not return correct boolean from isNodeTunnelReady",
                bridgeConfigurationManagerImpl.isNodeTunnelReady(any(Node.class), any(Node.class)));
        ...
        verify(configurationService, times(2)).isL3ForwardinfgEnabled();
        verify(configurationService, times(2)).getIntegrationBridgeName();
