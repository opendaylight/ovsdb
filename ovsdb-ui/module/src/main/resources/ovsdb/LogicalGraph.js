define(['app/ovsdb/lib/d3.min', 'app/ovsdb/OvsCore', 'underscore'], function(d3, OvsCore, _) {
  ...
  function addNetWorkRouterVmsToDom(networks, routers, vms) {
    ...
    d3Vm.append('svg:image')
      ...
      .attr('xlink:href', 'src/app/ovsdb/assets/vm.png');
  ...
  function positionateNetwork(network, x, y, margin) {
    ...
    network.y = y;
    margin = margin || 0;
    network.color = d3.hsl(Math.random() * 360, 1, 0.6).toString();
    ...
    var nbVm = network.vms.length;

    if (!network.external) {
      _.each(network.subnets, function(subnet) {
  ...
  function positionateVm(network, x, y) {
    ...
    _.each(network.vms, function(vm) {
