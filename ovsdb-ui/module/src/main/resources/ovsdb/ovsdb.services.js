...
define(['app/ovsdb/ovsdb.module', 'app/ovsdb/OvsCore', 'underscore', 'app/ovsdb/ovsdb.constant'], function(ovsdb, OvsCore, _) {
  ...
  ovsdb.register.factory('CacheFactory', function($cacheFactory, $q) {
    var svc = {},
              ...
              _.each(node[flowInfoKeys.TABLE], function(entry) {
                if (entry.id) {
                  ...
                  _.each(entry.flow, function(flow) {
                    bridgeNode.flowTable.push({ key: flow.table_id, value: flow.id});
                  });
                }
              });
            }
          ...
          _.each(topo.bridgeNodes, function(node, index) {
            _.each(node.tPs,function(tp, index) {
              ...
              if (tp.name.indexOf('vxlan') != -1) {
                ...
                var destNode = _.filter(topo.ovsdbNodes, function(node) {
                  return node.otherLocalIp  === destIP;
                });

                if (_.isEmpty(destNode)) {
                  return false;
                }
                topo.registerLink(new OvsCore.TunnelLink(tp.name, node.nodeId, destNode[0].nodeId));
              }
            });
          });
  ...
  var NeutronSvc = function(NeutronRestangular, CacheFactory, $q, $http) {
    ...
    svc.getNetworks = function() {
        return CacheFactory.obtainDataFromCache('network', fetchNetworks, this);
    };
  ...
  var OvsUtil = function(NeutronSvc, TopologySvc, CacheFactory, $q) {
    ...
    function extractLogicalTopology(cb) {
    ...
    svc.getLogicalTopology = function() {
      return CacheFactory.obtainDataFromCache('logicalTopology', extractLogicalTopology, this);
    };

    svc.extractLogicalByTenant = function(tenantId, subSet) {
      var portDefer = NeutronSvc.getPorts(),
        resultDefer = $q.defer();
      portDefer.then(function(data) {
        var filteredPorts = _.filter(data, function(p) {
    ...
    svc.extractLogicalBySubnet = function(subnets, subSet) {
      var portDefer = NeutronSvc.getPorts(),
        networkDefer = NeutronSvc.getNetworks(),
        resultDefer = $q.defer();
      $q.all([portDefer, networkDefer]).then(function(datas) {
        var ports = datas[0],
          networks = datas[1];
