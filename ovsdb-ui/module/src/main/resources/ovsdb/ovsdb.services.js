  ...
  var TopologySvc = function(OvsdbRestangular, nodeIdentifier, ovsNodeKeys, bridgeNodeKeys, tpKeys, flowInfoKeys, linkIdentifier, $q, $http) {
    ...
    svc.getTopologies = function(cb) {
      ...
      $q.all([invNodeDefer, netTopoDefer]).then(function(values) {
          ...
          if (!invNode || !invNode.nodes || !invNodeenodesenode) {
...
/*
  ...
  var NeutronSvc = function(NeutronRestangular, $cacheFactory, $q, $http) {
    ...
    function obtainDataFromCache(key, fn, ctx) {
      var ovsCache = null,
        cacheDefer = $q.defer();
    ...
    svc.getNetworks = function() {
        return obtainDataFromCache('network', fetchNetworks, this);
    };

    svc.getPorts = function() {
      return obtainDataFromCache('ports', fetchPorts, this);
    };

    svc.getRouters = function() {
      return obtainDataFromCache('routers', fetchRouters, this);
    };
  ...
  var OvsUtil = function(NeutronSvc, $q) {
    var svc = {};
    ...
    svc.extractLogicalTopology = function(cb) {
      ...
      $q.all([networksDefer, routersDefer, portsDefer]).then(function (datas) {
