...
import org.opendaylight.overlay.transactions.md.operational.OverlayOperationalCommandAggregator;
...
public class OverlayOperationalDataChangeListener implements DataChangeListener, AutoCloseable {

    private ListenerRegistration<DataChangeListener> registration;
    private DataBroker dbOvsdb;
    private TransactionInvoker txInvoker;
    private static final Logger LOG = LoggerFactory.getLogger(OverlayOperationalDataChangeListener.class);

    OverlayOperationalDataChangeListener(DataBroker db) {
        LOG.info("Registering OverlayOperationalDataChangeListener");
        InstanceIdentifier<Node> path = InstanceIdentifier
                .create(NetworkTopology.class)
                .child(Topology.class, new TopologyKey(OverlayConstants.OVSDB_TOPOLOGY_ID))
                .child(Node.class);
        registration =
                dbOvsdb.registerDataChangeListener(LogicalDatastoreType.OPERATIONAL, path,
    ...
    private void updateData(
        ...
        for (Node nodeInstance : nodeInstancesFromChanges(changes)) {
            transact(new OverlayOperationalCommandAggregator(
