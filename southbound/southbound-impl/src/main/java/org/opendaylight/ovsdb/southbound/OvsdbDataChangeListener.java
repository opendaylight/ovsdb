        ...
        for ( Entry<InstanceIdentifier<?>, DataObject> created : map.entrySet()) {
            if (created.getValue() instanceof Node) {
                ...
                if (client != null) {
                     ...
                     * multi threaded and i don't see any need to further parallelism per DataChange notifications processing..
                     */
                    if ( cm.getHasDeviceOwnership(client.getMDConnectionInfo())) {
                        LOG.debug("*this* instance of southbound plugin is an owner of the device {}",created.getValue());
                        result.put((InstanceIdentifier<Node>) created.getKey(), client);
                    } else {
                        LOG.debug("*this* instance of southbound plugin is not an owner of the device {}",created.getValue());
                    }
                } else {
