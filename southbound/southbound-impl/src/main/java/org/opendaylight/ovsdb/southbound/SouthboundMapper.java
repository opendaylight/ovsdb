...
public class SouthboundMapper {
    ...
    public static Map<Uri, Manager> createOvsdbManager(OvsdbNodeAugmentation ona,DatabaseSchema dbSchema) {
        List<ManagerEntry> managerEntries = ona.getManagerEntry();
        Map<Uri,Manager> managerMap = new HashMap<>();
        if (managerEntries != null && !managerEntries.isEmpty()) {
            for (ManagerEntry managerEntry : managerEntries) {
                Manager manager = TyperUtils.getTypedRowWrapper(dbSchema, Manager.class);
                manager.setTarget(managerEntry.getTarget().getValue());
                managerMap.put(new Uri(managerEntry.getTarget().getValue()), manager);
            }
        }
        return managerMap;
    }
