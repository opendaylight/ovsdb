...
public class SouthboundMapper {
    ...
    public static InstanceIdentifier<Node> createInstanceIdentifier(Map<String,String> externalIds,
            ConnectionInfo key) {
        InstanceIdentifier<Node> connectionIid = createInstanceIdentifier(key.getRemoteIp(), key.getRemotePort());
        if (externalIds != null) {
            if (externalIds.containsKey(IID_EXTERNAL_ID_KEY)) {
                connectionIid = (InstanceIdentifier<Node>) SouthboundUtil
                        .deserializeInstanceIdentifier(externalIds.get(IID_EXTERNAL_ID_KEY));
            } else if (externalIds.containsKey(EXTERNAL_IDS_OVS_SYSTEM_ID)) {
                connectionIid = SouthboundMapper.createInstanceIdentifier(externalIds.get(EXTERNAL_IDS_OVS_SYSTEM_ID));
            }
        }
