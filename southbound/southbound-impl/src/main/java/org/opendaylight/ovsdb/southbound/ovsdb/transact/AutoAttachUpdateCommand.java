...
public class AutoAttachUpdateCommand extends AbstractTransactCommand {
    ...
    public void execute(TransactionBuilder transaction) {
        ...
        // TODO: Fix after ovs community supports external_ids column in AutoAttach Table
        if (true) {
            LOG.debug("CRUD operations not supported from ODL for auto_attach column for"
    ...
    private void updateAutoAttach(TransactionBuilder transaction,
        ...
        if (autoAttachList != null) {
            for (Autoattach autoAttach : autoAttachList) {
                ...
                if (autoAttach.getAutoattachUuid() != null) {
                    UUID uuid = new UUID(getAutoAttachUuid(currentAutoAttach, autoAttach.getAutoattachUuid()).getValue());
