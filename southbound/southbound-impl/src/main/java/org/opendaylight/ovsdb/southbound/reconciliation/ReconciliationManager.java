...
public class ReconciliationManager implements AutoCloseable{
    ...
    public ReconciliationManager() {
        ...
        taskTriager = Executors.newScheduledThreadPool(Runtime.getRuntime().availableProcessors());
    }
        ...
        taskTriager.scheduleAtFixedRate(new Runnable() {
            ...
            public void run() {
                if(task.isTaskReadyForRetry()) {
                    ...
                    taskTriager.scheduleAtFixedRate(task,0,task.retryDelayInMills(),TimeUnit.MILLISECONDS);
                }
            }
        }, 0, task.retryDelayInMills(), TimeUnit.MILLISECONDS);
    }
