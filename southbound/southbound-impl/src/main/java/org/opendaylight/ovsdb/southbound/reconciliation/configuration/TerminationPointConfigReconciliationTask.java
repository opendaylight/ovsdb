...
public class TerminationPointConfigReconciliationTask extends ReconciliationTask {
    ...
    private final Map<InstanceIdentifier<OvsdbTerminationPointAugmentation>, OvsdbTerminationPointAugmentation> operTerminationPoints;
    ...
    public TerminationPointConfigReconciliationTask(final ReconciliationManager reconciliationManager,
            ...
            final Map<InstanceIdentifier<OvsdbTerminationPointAugmentation>, OvsdbTerminationPointAugmentation> operTerminationPoints,
    ...
    public boolean reconcileConfiguration(final OvsdbConnectionManager connectionManager) {
        ...
        LOG.trace("delete Reconciliation: Triggered {}", configNodeData.getTerminationPoint());
        ...
        LOG.trace("delete Reconciliation: configTerminationPoints {}", configTerminationPoints);
        ...
        LOG.trace("delete Reconciliation: operTerminationPoints {}", operTerminationPoints.size());
        for (Map.Entry<InstanceIdentifier<OvsdbTerminationPointAugmentation>, OvsdbTerminationPointAugmentation> entry :
            ...
            LOG.trace("delete Reconciliation: {}", entry.getKey());
            ...
            if (configTerminationPoints.contains(terminationPoint.getName())) {
                LOG.trace("delete Reconciliation: Termination Point {} present in config DS", terminationPoint.getName());
            } else {
                LOG.trace("delete Reconciliation:Termination Point {} not present in config DS, checking if odl_created", terminationPoint.getName());
                List<PortExternalIds> externalIds = terminationPoint.getPortExternalIds();
                if (externalIds != null) {
                    for (PortExternalIds portExternalIds : externalIds) {
                        if (portExternalIds.getExternalIdKey().equals("created_by") && portExternalIds.getExternalIdValue().equals("odl")) {
                            LOG.trace("delete Reconciliation:Termination Point {} created by ODL. Marking for deletion during reconciliation", entry.getKey());
        ...
        DataChangeEvent deleteChangeEvents = new DataChangeEvent() {
            @Override
            public Map<InstanceIdentifier<?>, DataObject> getCreatedData() { return Collections.emptyMap(); }
            ...
            @Override
            public Map<InstanceIdentifier<?>, DataObject> getOriginalData() { return original; }
