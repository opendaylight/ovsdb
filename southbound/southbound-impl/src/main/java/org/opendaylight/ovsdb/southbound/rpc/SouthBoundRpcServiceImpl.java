...
public class SouthBoundRpcServiceImpl implements SouthBoundRpcService {
  ...
  public ListenableFuture<RpcResult<ConfigureTerminationPointWithQosOutput>> configureTerminationPointWithQos(
    ...
    LOG.trace("configureTerminationPointWithQos called for {} for configure ingressPolicyBurst {}, ingressPolicyRate {} on node {}",
        terminationPointStr, ingressPolicyBurst, ingressPolicyRate, ((org.opendaylight.yang.gen.v1.urn.tbd.params.xml.ns.yang.network.topology.rev131021.NodeId) parentNodeId).toString());
    ...
    if (ovsdbNodeOpt.isPresent()) {
      ...
      if (isterminationPointPresentinOper) {
        OvsdbConnectionInstance connectionInstance =  southboundProvider.getCm().getConnectionInstance(parentNodeIid);
        if (connectionInstance != null && connectionInstance.getHasDeviceOwnership()) {
          ...
          connectionInstance
              .updateTerminationPointWithQosParameters(terminationPointStr, ingressPolicyBurst,
                  ingressPolicyRate);
        }
      } else {
