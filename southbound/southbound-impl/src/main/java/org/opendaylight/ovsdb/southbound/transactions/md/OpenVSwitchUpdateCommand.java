...
public class OpenVSwitchUpdateCommand extends AbstractTransactionComponent {
    ...
    public void execute(final OvsdbTransactionContext context) {
        final Map<UUID, OpenVSwitch> deletedOpenVSwitchRows = context.getOldRows(OpenVSwitch.class);
        for (Entry<UUID, OpenVSwitch> entry : context.getUpdatedRows(OpenVSwitch.class).entrySet()) {
            ...
            // TODO: it seems we really would like old/new view
            OpenVSwitch oldEntry = deletedOpenVSwitchRows.get(entry.getKey());
            setExternalIds(context.getTransaction(), ovsdbNodeBuilder, oldEntry, openVSwitch);
            setOtherConfig(context.getTransaction(), ovsdbNodeBuilder, oldEntry, openVSwitch);
            ovsdbNodeBuilder.setConnectionInfo(context.getConnectionInfo());
            ...
            context.getTransaction().merge(LogicalDatastoreType.OPERATIONAL, nodePath, nodeBuilder.build());
        }
    }
