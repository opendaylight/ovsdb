...
public class OvsdbAutoAttachUpdateCommand extends TransactionComponent {
    ...
    public void execute(final ReadWriteTransaction transaction) {
        final Map<UUID, AutoAttach> updatedAutoAttachRows = TyperUtils.extractRowsUpdated(AutoAttach.class, getUpdates(), getDbSchema());
        final Map<UUID, AutoAttach> oldAutoAttachRows = TyperUtils.extractRowsOld(AutoAttach.class, getUpdates(), getDbSchema());
...
//                setExternalIds(autoAttachBuilder, autoAttach);
    ...
    private void setMappings(final AutoattachBuilder autoAttachBuilder,
