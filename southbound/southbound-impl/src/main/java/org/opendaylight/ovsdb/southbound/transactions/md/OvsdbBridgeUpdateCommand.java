...
public class OvsdbBridgeUpdateCommand extends AbstractTransactionComponent {
    private static final Logger LOG = LoggerFactory.getLogger(OvsdbBridgeUpdateCommand.class);

    public OvsdbBridgeUpdateCommand() {
        super(Bridge.class);
    }

    @Override
    public void execute(final OvsdbTransactionContext context) {
        Map<UUID, Bridge> updatedBridgeRows = context.getUpdatedRows(Bridge.class);
        if (updatedBridgeRows == null || updatedBridgeRows.isEmpty()) {
            return;
        }

        final Map<UUID, Bridge> oldBridgeRows = context.getOldRows(Bridge.class);
        final InstanceIdentifier<Node> connectionIId = context.getNode();
        Optional<Node> connection = SouthboundUtil.readNode(transaction, connectionIId);
        if (!connection.isPresent()) {
            return;
        }

        final List<InstanceIdentifier<Node>> updatedBridges = new ArrayList<>();
    ...
    private static void setAutoAttach(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
        try {
    ...
    private static void setDataPathType(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
        ovsdbBridgeAugmentationBuilder.setDatapathType(
                SouthboundMapper.createDatapathType(bridge.getDatapathTypeColumn().getData()));
    }

    private static void setFailMode(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
    ...
    private static void setOtherConfig(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
    ...
    private static void setExternalIds(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
    ...
    private static void setProtocol(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
    ...
    private static void setDataPath(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
    ...
    private void setOpenFlowNodeRef(final OvsdbBridgeAugmentationBuilder ovsdbBridgeAugmentationBuilder, final Bridge bridge) {
