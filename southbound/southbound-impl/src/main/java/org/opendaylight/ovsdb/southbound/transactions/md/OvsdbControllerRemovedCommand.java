...
public class OvsdbControllerRemovedCommand extends AbstractTransactionComponent {
    ...
    public void execute(final OvsdbTransactionContext context) {
        Map<UUID, Bridge> oldBridgeRows = context.getOldRows(Bridge.class);
        Map<UUID, Controller> removedControllerRows = context.getRemovedRows(Controller.class);
        for (Bridge bridge : context.getUpdatedRows(Bridge.class).values()) {
