...
public class OvsdbInterfaceUpdateCommand extends AbstractTransactionCommand {
    ...
    public void execute(ReadWriteTransaction transaction) {
        Collection<Interface> updatedRows = TyperUtils.extractRowsUpdated(Interface.class, getUpdates(), getDbSchema()).values();
            for(Interface intface : updatedRows) {
            ...
            if(node.isPresent()){
                ...
                NodeId manageNodeId = SouthboundMapper.createManagedNodeId(getKey(), intface.getUuid());
                ...
                ovsdbTerminationPointBuilder.setName(intface.getName());
                ovsdbTerminationPointBuilder.setInterfaceUuid(new Uuid(intface.getUuid().toString()));
                //ovsdbTerminationPointBuilder.setType(intface.getInterfaceType());
                //ovsdbTerminationPointBuilder.setType(new org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.ovsdb.rev150105.InterfaceTypeBase(intface.get));                               
                ...
                LOG.debug("Update node with TerminationPoint Interface ref {}",ovsdbTerminationBuilder.toString());
