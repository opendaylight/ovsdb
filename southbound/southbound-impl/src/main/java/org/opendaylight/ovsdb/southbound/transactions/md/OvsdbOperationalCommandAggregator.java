...
public class OvsdbOperationalCommandAggregator implements TransactionCommand {
    ...
    private static final ImmutableList<AbstractTransactionComponent> BASE = ImmutableList.of(
    ...
    private static final ImmutableList<AbstractTransactionComponent> AUTOATTACH =
            ImmutableList.<AbstractTransactionComponent>builderWithExpectedSize(BASE.size() + 2)
                .addAll(BASE)
    ...
    private final ImmutableList<AbstractTransactionComponent> components;
    ...
    public OvsdbOperationalCommandAggregator(final InstanceIdentifierCodec instanceIdentifierCodec,
            final OvsdbConnectionInstance connection, final TableUpdates updates, final DatabaseSchema dbSchema) {
        ...
        this.dbSchema = TypedDatabaseSchema.of(dbSchema);
    ...
    OvsdbOperationalCommandAggregator(final InstanceIdentifierCodec instanceIdentifierCodec,
            final OvsdbConnectionInstance connection, final TableUpdates updates, final DatabaseSchema dbSchema,
            final AbstractTransactionComponent component) {
    ...
    public void execute(final ReadWriteTransaction transaction) {
        ...
        final OvsdbTransactionContext context = new OvsdbTransactionContext(dbSchema, updates, connection, transaction);
        for (AbstractTransactionComponent component : components) {
            try {
                component.execute(context);
            } catch (NullPointerException | NoSuchElementException | ClassCastException e) {
                LOG.warn("Exception trying to execute component {}", component, e);
            }
        }
    }
