...
public class OvsdbPortUpdateCommand extends AbstractTransactionCommand {
    ...
    public OvsdbPortUpdateCommand(OvsdbConnectionInstance key,
            TableUpdates updates, DatabaseSchema dbSchema) {
        super(key, updates, dbSchema);
        portUpdatedRows = TyperUtils.extractRowsUpdated(Port.class, updates,
                dbSchema);
        portOldRows = TyperUtils.extractRowsOld(Port.class, updates, dbSchema);
        interfaceUpdatedRows = TyperUtils.extractRowsUpdated(Interface.class,
                updates, dbSchema);
        interfaceOldRows = TyperUtils.extractRowsOld(Interface.class, updates,
                dbSchema);
        bridgeUpdatedRows = TyperUtils.extractRowsUpdated(Bridge.class,
                updates, dbSchema);
        portQosUpdatedRows = TyperUtils.extractRowsUpdated(Qos.class, updates,
                dbSchema);
        portQueueUpdatedRows = TyperUtils.extractRowsUpdated(Queue.class, updates,
                dbSchema);
    }

    @Override
    public void execute(ReadWriteTransaction transaction) {
        final InstanceIdentifier<Node> connectionIId = getOvsdbConnectionInstance()
    ...
    private void updateTerminationPoints(ReadWriteTransaction transaction,
            Node node) {
        for (Entry<UUID, Port> portUpdate : portUpdatedRows.entrySet()) {
            String portName = null;
            portName = portUpdate.getValue().getNameColumn().getData();
            Optional<InstanceIdentifier<Node>> bridgeIid = getTerminationPointBridge(portUpdate
                    .getKey());
            if (!bridgeIid.isPresent()) {
                bridgeIid = getTerminationPointBridge(transaction, node,
                        portName);
            }
            if (bridgeIid.isPresent()) {
                NodeId bridgeId = SouthboundMapper
                        .createManagedNodeId(bridgeIid.get());
                TerminationPointKey tpKey = new TerminationPointKey(new TpId(
                        portName));
                TerminationPointBuilder tpBuilder = new TerminationPointBuilder();
                tpBuilder.setKey(tpKey);
                tpBuilder.setTpId(tpKey.getTpId());
                InstanceIdentifier<TerminationPoint> tpPath = getInstanceIdentifier(
                        bridgeIid.get(), portUpdate.getValue());
                OvsdbTerminationPointAugmentationBuilder tpAugmentationBuilder
                    = new OvsdbTerminationPointAugmentationBuilder();
                buildTerminationPoint(tpAugmentationBuilder,
                        portUpdate.getValue());
                UUID interfaceUUID = (UUID) portUpdate.getValue()
                        .getInterfacesColumn().getData().toArray()[0];
                ...
                tpBuilder.addAugmentation(
                        OvsdbTerminationPointAugmentation.class,
                        tpAugmentationBuilder.build());
                if (portOldRows.containsKey(portUpdate.getKey())) {
                    transaction.merge(LogicalDatastoreType.OPERATIONAL, tpPath,
                            tpBuilder.build());
                } else {
                    transaction.put(LogicalDatastoreType.OPERATIONAL, tpPath,
                            tpBuilder.build());
                }
            }
        }
        for (Entry<UUID, Interface> interfaceUpdate : interfaceUpdatedRows
                .entrySet()) {
            String interfaceName = null;
            interfaceName = interfaceUpdatedRows.get(interfaceUpdate.getKey())
                    .getNameColumn().getData();
            Optional<InstanceIdentifier<Node>> bridgeIid = getTerminationPointBridge(
                    transaction, node, interfaceName);
            if (bridgeIid.isPresent()) {
                NodeId bridgeId = SouthboundMapper
                        .createManagedNodeId(bridgeIid.get());
                TerminationPointKey tpKey = new TerminationPointKey(new TpId(
    ...
    private void buildTerminationPoint(
        ...
        tpAugmentationBuilder.setName(portUpdate.getName());
        tpAugmentationBuilder.setPortUuid(new Uuid(portUpdate.getUuid()
                .toString()));
    ...
    private void buildTerminationPoint(
        ...
        tpAugmentationBuilder.setName(interfaceUpdate.getName());
        tpAugmentationBuilder.setInterfaceUuid(new Uuid(interfaceUpdate
                .getUuid().toString()));
        updateInterfaces(interfaceUpdate, tpAugmentationBuilder);
    }

    private Optional<Node> readNode(final ReadWriteTransaction transaction,
    ...
    private Optional<InstanceIdentifier<Node>> getTerminationPointBridge(
        ...
        for (UUID bridgeUUID : this.bridgeUpdatedRows.keySet()) {
            if (this.bridgeUpdatedRows.get(bridgeUUID).getPortsColumn()
                    .getData().contains(portUUID)) {
                return Optional.of(SouthboundMapper.createInstanceIdentifier(
    ...
    private Optional<InstanceIdentifier<Node>> getTerminationPointBridge(
        ...
        OvsdbNodeAugmentation ovsdbNode = node
        ...
        for (ManagedNodeEntry managedNodeEntry : managedNodes) {
            ...
            Node managedNode = readNode(
                    transaction,
                    (InstanceIdentifier<Node>) managedNodeEntry.getBridgeRef()
                            .getValue()).get();
            TerminationPointBuilder tpBuilder = new TerminationPointBuilder();
            ...
            if (managedNode.getTerminationPoint().contains(tpBuilder.build())) {
                OvsdbBridgeAugmentation ovsdbNodeAugment = managedNode
                        .getAugmentation(OvsdbBridgeAugmentation.class);
                return Optional.of((InstanceIdentifier<Node>) managedNodeEntry
                        .getBridgeRef().getValue());
            }
        }
        return Optional.absent();
    }
    ...
    private void updateInterfaces(
        ...
        Column<GenericTableSchema, String> typeColumn = interfaceUpdate
                .getTypeColumn();
    ...
    private void updateInterface(
        ...
        ovsdbTerminationPointBuilder.setInterfaceUuid(new Uuid(interf.getUuid()
                .toString()));
        ovsdbTerminationPointBuilder.setInterfaceType(SouthboundMapper
                .createInterfaceType(type));
    ...
    private void updateVlanTrunks(
        ...
        if (!portTrunks.isEmpty()) {
            for (Long trunk : portTrunks) {
                if (trunk != null) {
                    modelTrunks.add(new TrunksBuilder().setTrunk(
                            new VlanId(trunk.intValue())).build());
                }
            }
        ...
        if (!vlanMode.isEmpty()) {
            ...
            if (vlanType.equals(SouthboundConstants.VLANMODES.ACCESS.getMode())) {
                ovsdbTerminationPointBuilder
                    ...
                    .equals(SouthboundConstants.VLANMODES.NATIVE_TAGGED
                            .getMode())) {
                ovsdbTerminationPointBuilder
                        .setVlanMode(OvsdbPortInterfaceAttributes.VlanMode.NativeTagged);
            } else if (vlanType
                    .equals(SouthboundConstants.VLANMODES.NATIVE_UNTAGGED
                            .getMode())) {
                ovsdbTerminationPointBuilder
                        .setVlanMode(OvsdbPortInterfaceAttributes.VlanMode.NativeUntagged);
            } else if (vlanType.equals(SouthboundConstants.VLANMODES.TRUNK
                    .getMode())) {
                ovsdbTerminationPointBuilder
    ...
    private void updateOfPort(
        ...
        if (ofPorts != null && !ofPorts.isEmpty()) {
            ...
            if (ofPort >= 0) {
                ovsdbTerminationPointBuilder.setOfport(ofPort);
            } else {
                LOG.debug(
                        "Received negative value for ofPort from ovsdb for {} {} {}",
    ...
    private void updateOfPortRequest(
        ...
        if (ofPortRequests != null && !ofPortRequests.isEmpty()) {
            ...
            if (ofPort >= 0) {
                ovsdbTerminationPointBuilder.setOfportRequest(ofPort);
            } else {
                LOG.debug(
                        "Received negative value for ofPort from ovsdb for {} {} {}",
                        interf.getName(), ofPort);
    ...
    private void updateInterfaceExternalIds(
        ...
        Map<String, String> interfaceExternalIds = interf
        ...
        if (interfaceExternalIds != null && !interfaceExternalIds.isEmpty()) {
            ...
            List<InterfaceExternalIds> externalIdsList = new ArrayList<InterfaceExternalIds>();
            ...
            ovsdbTerminationPointBuilder
                    .setInterfaceExternalIds(externalIdsList);
        }
    }
    ...
    private void updatePortExternalIds(
        ...
        Map<String, String> portExternalIds = port.getExternalIdsColumn()
                .getData();
        ...
        if (optionsMap != null && !optionsMap.isEmpty()) {
            ...
            for (String optionsKeyString : optionsMap.keySet()) {
                ...
                if (optionsKeyString != null && optionsValueString != null) {
                    ...
                    options.add(new OptionsBuilder().setKey(optionsKey)
    ...
    private void updatePortOtherConfig(
        ...
        Map<String, String> portOtherConfigMap = port.getOtherConfigColumn()
                .getData();
        if (portOtherConfigMap != null && !portOtherConfigMap.isEmpty()) {
            ...
            for (String portOtherConfigKeyString : portOtherConfigMap.keySet()) {
                portOtherConfigValueString = portOtherConfigMap
                        .get(portOtherConfigKeyString);
                if (portOtherConfigKeyString != null
                        && portOtherConfigValueString != null) {
                    portOtherConfigs.add(new PortOtherConfigsBuilder()
                            .setOtherConfigKey(portOtherConfigKeyString)
                            .setOtherConfigValue(portOtherConfigValueString)
                            .build());
                }
            }
    ...
    private void updateInterfaceOtherConfig(
        ...
        Map<String, String> interfaceOtherConfigMap = interf
                .getOtherConfigColumn().getData();
        if (interfaceOtherConfigMap != null
                && !interfaceOtherConfigMap.isEmpty()) {
            ...
            for (String interfaceOtherConfigKeyString : interfaceOtherConfigMap
                    .keySet()) {
                interfaceOtherConfigValueString = interfaceOtherConfigMap
                        .get(interfaceOtherConfigKeyString);
                if (interfaceOtherConfigKeyString != null
                        && interfaceOtherConfigValueString != null) {
                    interfaceOtherConfigs
                            .add(new InterfaceOtherConfigsBuilder()
                                    .setOtherConfigKey(
                                            interfaceOtherConfigKeyString)
                                    .setOtherConfigValue(
                                            interfaceOtherConfigValueString)
                                    .build());
                }
            }
            ovsdbTerminationPointBuilder
                    .setInterfaceOtherConfigs(interfaceOtherConfigs);
        }
    }

    private InstanceIdentifier<TerminationPoint> getInstanceIdentifier(
            InstanceIdentifier<Node> bridgeIid, Port port) {
        if (port.getExternalIdsColumn() != null
                && port.getExternalIdsColumn().getData() != null
                && port.getExternalIdsColumn().getData()
                        .containsKey(SouthboundConstants.IID_EXTERNAL_ID_KEY)) {
            String iidString = port.getExternalIdsColumn().getData()
                    .get(SouthboundConstants.IID_EXTERNAL_ID_KEY);
            return (InstanceIdentifier<TerminationPoint>) SouthboundUtil
                    .deserializeInstanceIdentifier(iidString);
        } else {
            return bridgeIid.child(TerminationPoint.class,
                    new TerminationPointKey(new TpId(port.getName())));
        }
    }
}
