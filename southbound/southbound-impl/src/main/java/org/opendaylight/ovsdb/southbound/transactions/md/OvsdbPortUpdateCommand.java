...
public class OvsdbPortUpdateCommand extends AbstractTransactionCommand {
    ...
    private void updateQos(final ReadWriteTransaction transaction, final Node node, InstanceIdentifier<TerminationPoint> tpPath,
        ...
        if (port.getValue().getQosColumn() == null) {
            return;
        }
        ...
        if (!qosUuidCol.isEmpty()) {
            ...
            Uri qosId = getQosId(ovsdbNode, qosUuid);
            ...
            if (portOldRows.containsKey(port.getKey()) &&
                ...
                if (!oldQos.isEmpty()) {
                    ...
                    if (!oldQosUuid.equals(qosUuid)) {
                        InstanceIdentifier<QosEntry> qosIId = tpPath
                            .augmentation(OvsdbTerminationPointAugmentation.class)
                            .child(QosEntry.class,
                                new QosEntryKey(getQosId(ovsdbNode, oldQosUuid)));
                        transaction.delete(LogicalDatastoreType.OPERATIONAL, qosIId);
                    }
                }
            }
            ...
            qosList.add(new QosEntryBuilder()
                .setKey(new QosEntryKey(qosId))
                .setQosId(qosId).build());
            ovsdbTerminationPointBuilder.setQosEntry(qosList);
        }
    }

    private Uri getQosId(OvsdbNodeAugmentation ovsdbNode, UUID qosUuid) {
        ...
        for (QosEntries qosEntry : ovsdbNode.getQosEntries()) {
            if (qosEntry.getQosUuid().equals(new Uuid(qosUuid.toString()))) {
                return qosEntry.getQosId();
            }
        }

        // Search for QoS entry in the current OVS updates
        for (Entry<UUID, Qos> qosUpdate : qosUpdatedRows.entrySet()) {
            Qos qos = qosUpdate.getValue();
            if (qos.getUuid().equals(qosUuid)) {
                if (qos.getExternalIdsColumn().getData().containsKey(SouthboundConstants.QOS_ID_EXTERNAL_ID_KEY)) {
                    return new Uri(qos.getExternalIdsColumn().getData().get(SouthboundConstants.QOS_ID_EXTERNAL_ID_KEY));
                } else {
                    return new Uri(SouthboundConstants.QOS_URI_PREFIX + "://" + qosUuid.toString());
                }
            }
        }
