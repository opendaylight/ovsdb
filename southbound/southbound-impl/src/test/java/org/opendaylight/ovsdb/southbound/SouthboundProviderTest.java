...
public class SouthboundProviderTest {
    ...
    @SuppressWarnings("unchecked")
    @Test //TO-DO StackOverflow, issue mocking: InstanceIdentifier<Topology> path = InstanceIdentifier.create(NetworkTopology.class).child(Topology.class, new TopologyKey(SouthboundConstants.OVSDB_TOPOLOGY_ID));
    public void testInitializeOvsdbTopology() throws Exception {
        InstanceIdentifier<Topology> path = mock(KeyedInstanceIdentifier.class);
        PowerMockito.mockStatic(InstanceIdentifier.class);
        InstanceIdentifier<NetworkTopology> iidNetworkToplogy = PowerMockito.mock(InstanceIdentifier.class);
        TopologyKey topologyKey = mock(TopologyKey.class);
        PowerMockito.whenNew(TopologyKey.class).withParameterTypes(TopologyId.class).withArguments(any(TopologyId.class)).thenReturn(topologyKey);
        when(InstanceIdentifier.create(NetworkTopology.class)).thenReturn(iidNetworkToplogy);
        PowerMockito.when(iidNetworkToplogy.child(Topology.class, topologyKey)).thenReturn((KeyedInstanceIdentifier<Topology, TopologyKey>) path);
