    ...
    public void testCRUDAutoAttach() throws InterruptedException {
        ...
        try (TestBridge testBridge = new TestBridge(connectionInfo, SouthboundITConstants.BRIDGE_NAME)) {
            ...
            try(TestAutoAttach testAutoattach = new TestAutoAttach(connectionInfo, new Uri(testAutoattachId),
                ...
                assertNotNullWithVersionCheck(operAa, isOldSchema);
                // FIXME: Check if isOldSchema is true after CRUD is supported for Autoattach
                if (operAa == null) {
                    // skip tests after verifying that Autoattach doesn't break with unsupported schema
                    // FIXME: Uncomment the below line
//                    Assume.assumeTrue(!isOldSchema);
    ...
    private void assertNotNullWithVersionCheck(Object obj, boolean isOldSchema) {
        if(!isOldSchema) {
            // FIXME: Uncomment after ovs supports external_ids column to test CRUD
