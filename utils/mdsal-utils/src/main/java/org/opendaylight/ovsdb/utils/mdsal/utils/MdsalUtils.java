...
public class MdsalUtils {
     ...
     * @param instanceIdentifiers
     *            the instanceIdentifiers
     * @return the result of the request
     */
    public <D extends org.opendaylight.yangtools.yang.binding.DataObject> boolean delete(
            final LogicalDatastoreType store, final Iterable<InstanceIdentifier<D>> instanceIdentifiers) {
        boolean result = false;
        final WriteTransaction transaction = databroker.newWriteOnlyTransaction();
        for (InstanceIdentifier<D> path : instanceIdentifiers) {
            transaction.delete(store, path);
        }
        CheckedFuture<Void, TransactionCommitFailedException> future = transaction.submit();
        try {
            ...
            LOG.warn("Failed to delete {} ", instanceIdentifiers, e);
        }
        return result;
    }
