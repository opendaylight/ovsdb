...
public class DockerOvs implements AutoCloseable {
    ...
    public void close() throws Exception {
        if (isRunning) {
            ProcUtils.runProcess(5000, downCmd);
            isRunning = false;
        }
    ...
    public void logState() throws IOException, InterruptedException {
        tryInContainer(5000, 0, "ip", "addr");
        tryInContainer(5000, 0, "ovs-vsctl", "show");
        tryInContainer(5000, 0, "ovs-ofctl", "-OOpenFlow13", "show", "br-int");
        tryInContainer(5000, 0, "ovs-ofctl", "-OOpenFlow13", "dump-flows", "br-int");
        tryInContainer(5000, 0, "ip", "netns", "list");
    }
